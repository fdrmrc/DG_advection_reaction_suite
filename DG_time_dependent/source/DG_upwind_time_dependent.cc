...
double RightHandSide<dim>::value(const Point<dim> &p,
	...
	(void) component;
	return M_PI*(p[0]*sin(M_PI*p[0])*sin(M_PI*p[1]) + p[1]*cos(M_PI*p[0])*cos(M_PI*p[1]) ) + sin(p[0]*M_PI)*cos(M_PI*p[1]);
}
...
double Solution<dim>::value(const Point<dim> &p, const unsigned int) const {
	...
	return sin(M_PI*p[1])*cos(M_PI*p[0]); //sin(pi*x)*cos(pi*y)
}
...
//	}
	...
	ode.explicit_function = [&](double t, const VectorType &y,
			VectorType &ydot) -> int {
		system_matrix.vmult(ydot, y);
		ydot*=-1;
		unsigned int m = right_hand_side.size();
		for(unsigned int i=0;i<m;++i){
			right_hand_side[i]*=std::exp(t);
		}

		ydot += right_hand_side;
		return 0;
	};



	Vector<double> y(system_matrix.m());

	ode.output_step = [&](const double t, const VectorType &sol,
			const unsigned int step_number) -> int {
		std::cout <<"Time: " <<t << "\t" << sol.norm_sqr()<< "\n";



		const std::string filename = "solution-at-time" + std::to_string(t) + ".vtk";
		std::cout << "  Writing solution to <" << filename << ">" << std::endl;
		std::ofstream output(filename);

		DataOut<dim> data_out;
		data_out.attach_dof_handler(dof_handler);
		data_out.add_data_vector(y, "u", DataOut<dim>::type_dof_data);
